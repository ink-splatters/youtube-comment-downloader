...
@click.command()
@click.option('--youtubeid', '-y', help='ID of Youtube video for which to download the comments')
@click.option('--url', '-u', help='Youtube URL for which to download the comments')
@click.option('--output', '-o', required=True, help='Output filename (output format is line delimited JSON)')
@click.option('--pretty/--no-pretty', '-p', default=False, help='Change the output format to indented JSON')
@click.option('--limit', '-l', type=int, help='Limit the number of comments')
@click.option('--language', '-a', type=str, default=None, help='Language for Youtube generated text (e.g. en)')
@click.option('--sort', '-s', type=int, default=SORT_BY_RECENT,
              help='Whether to download popular (0) or recent comments (1). Defaults to 1')
def main(youtubeid: Optional[str], url: Optional[str], output: str, pretty: bool, limit: Optional[int], language: Optional[str], sort: int) -> None:
    ...
    with io.open(output, 'w', encoding='utf8') as fp:
        ...
        while comment:
            ...
            print(comment_str.decode('utf-8') if isinstance(comment_str, bytes) else comment_str, file=fp, end='')
